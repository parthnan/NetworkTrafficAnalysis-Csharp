...
{
    public class MD1withPriorityFix
    {
        #region  乱数 (サイコロ）関連
        private Random rnd;
		...
		public List<Person> remaining = new List<Person>();
        public List<Person> remaining_priority = new List<Person>();
        ...
        //コンストラクター。
        //オブジェクトを作る時に呼び出される関数。
        public MD1withPriorityFix(double arrivalrate,double arrivalrate_prior, int seed = 0, int n = 1000)
        {
        ...
        public void run()
        {
            ...
            while (true)
            {
                ...
                if (current == null&&remaining.Count==0&&remaining_priority.Count==0) break;
                #endregion
                ...
                if (current.action.Equals("ARRIVE"))
                {
                    ...
                    if (current.who.isvip)
                    {
						...
						remaining_priority.Add(current.who);
                        if (!ServerInProcess)
