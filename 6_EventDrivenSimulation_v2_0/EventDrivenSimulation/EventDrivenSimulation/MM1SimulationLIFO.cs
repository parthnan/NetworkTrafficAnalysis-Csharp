...
{
    public class MM1SimulationLIFO
    {
        #region  乱数 (サイコロ）関連
        private Random rnd;
        ...
        //コンストラクター。
        //オブジェクトを作る時に呼び出される関数。
        public MM1SimulationLIFO(double arrivalrate, int seed=0, int n=1000)
        {
        ...
        public void run()
        {
            ...
            while (true)
            {
				...
				else if (current.action.Equals("BEGINPROCESS"))
                {
                    ...
                    {
						i=0;
						while(real_queue[i].arrivaltime<=this.simtime&&i<real_queue.Count-1){
							i++;
						}
                        Person head = real_queue[i];//参照渡し？値渡し?
                        real_queue.RemoveAt(i);
						double servicetime = this.ExponentialDistribution(1.0); //平均1の指数分布
						double wtime = queue_length + servicetime; //離脱のイベントを追加
                        this.elist.Add(new MyEvent(this.simtime + queue_length +this.ExponentialDistribution(1.0), 0, head, "ENDPROCESS"));
                    }
                }
